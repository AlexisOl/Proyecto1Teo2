...
export class GeneralUsuarioComponent implements OnInit {
  ...
  todosAyudasVoluntariados: any;
  facturasDetalle: any = [];
  voluntariadosDetalle: any = [];
  chart: any;
  facturas: any;
  insginias: any;
  cupones: any;
  ...
  obtenerAyudaVoluntariado() {
    this.voluntariadoServicio
      .obtenerVoluntariadoId(this.sesionServicio.getUsuario()?.id)
      .pipe(
        switchMap((ayudaVoluntariado: any) => {
          this.todosAyudasVoluntariados = ayudaVoluntariado;
          console.log(ayudaVoluntariado, 'a');

          return this.obtenerDetalleVoluntariado();
        })
      )
      .subscribe();
  }

  obtenerDetalleVoluntariado() {
    return this.todosAyudasVoluntariados.map((valores: any) => {
      console.log(valores.id);

      //ahora si retorna todo
      return this.voluntariadoServicio
        .obtenerVoluntariadoIdDetalle(
          this.sesionServicio.getUsuario()?.id,
          valores.id
        )
        .subscribe((valores: any) => {
          console.log(valores);
          this.voluntariadosDetalle.push(valores);
        });
    });
  }

  obtenerLasFacturas() {
    this.comprarServicio
      .obtenerFacturasId(this.sesionServicio.getUsuario()?.id)
      .pipe(
  ...
  detalleFactura() {
    return this.todasLasFacturas.map((valores: any) => {
      ...
      //ahora si retorna todo
      return this.comprarServicio
        .obtenerFacturasIdDetalle(
          this.sesionServicio.getUsuario()?.id,
          valores.id
        )
        .subscribe((valores: any) => {
  ...
  generarGrafico() {
    this.comprarServicio
      .obtenerIdFacturasTotal(this.sesionServicio.getUsuario()?.id)
    ...
    this.chart = new Chart('forma', {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Precio Total',
            data: this.facturas.map(
              (row: { precioTotal: any }) => row.precioTotal
            ),
          },
        ],
      },
  ...
  ngOnInit(): void {
    ...
    console.log(this.sesionServicio.getUsuario()?.password);
    this.obtenerLasFacturas();
    this.obtenerAyudaVoluntariado();
    this.generarGrafico();

    //funcion para obtener insignias
    this.voluntariadoServicio
      .obtenerInsignia(this.sesionServicio.getUsuario()?.id)
      .subscribe((elementos: insignias) => {
